...
class CustomNuScenesLocalMapDataset(CustomNuScenesDataset):
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        # print(example)
        example['gt_labels_3d_fus'] = example['gt_labels_3d']
        example['gt_bboxes_3d_fus'] = example['gt_bboxes_3d']
    ...
    def prepare_train_data(self, index):
        ...
        if self.filter_empty_gt and \
                ((example is None or ~(example['gt_labels_3d']._data != -1).any()) or \
                    (example is None or ~(example['gt_labels_3d_fus']._data != -1).any())):
            return None
        ...
        return self.union2one_map(data_queue)

    def union2one_map(self, queue):
        """
        ...
        for i in range(len(pts_list)-1):
            queue[-1]['points'+str(i)] = DC(pts_list[i])
        # queue[-1]['points'] = DC(torch.cat(pts_list),
        #                       cpu_only=False, stack=True)
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            #     f.write(f'Index in nuscenes_map_dataset {idx} \n')
    ...
    def _format_bbox_obj(self, results, jsonfile_prefix=None):
            """Convert the results to the standard format.
    ...
    def evaluate(self,
        ...
        """
        result_files, result_files_obj, tmp_dir = self.format_results(results, jsonfile_prefix)
        ...
        result_name_obj =['pts_bbox_obj']


        if isinstance(result_files_obj, dict):
            results_dict_obj = dict()
            for name in result_name_obj:
                print('Evaluating bboxes of {}'.format(name))
                ret_dict = self._evaluate_single_obj(result_files_obj[name])
            results_dict_obj.update(ret_dict)
        elif isinstance(result_files_obj, str):
            results_dict_obj = self._evaluate_single_obj(result_files_obj)
        ...
        results_dict.update(results_dict_obj)
