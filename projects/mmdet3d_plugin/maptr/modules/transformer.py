...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.decoder_obj = build_transformer_layer_sequence(decoder_obj)
    ...
    def init_layers(self):
        ...
        self.reference_points1 = nn.Linear(self.embed_dims, 3)
    ...
    def init_weights(self):
        ...
        xavier_init(self.reference_points1, distribution='uniform', bias=0.)
    ...
    def get_decoder_features(self,
                ...
                bev_h,
                bev_w,
                reg_branches_obj=None,               
                cls_branches_obj=None,
                prev_bev = None,
                **kwargs):

        # bs = mlvl_feats[0].size(0)
        # query_pos_obj, query_obj = torch.split(
        #     object_query_embeds_obj, self.embed_dims, dim=1)
        # query_pos_obj = query_pos_obj.unsqueeze(0).expand(bs, -1, -1)
        # query_obj = query_obj.unsqueeze(0).expand(bs, -1, -1)
        # reference_points_obj = self.reference_points1(query_pos_obj)
        # reference_points_obj = reference_points_obj.sigmoid()
        # init_reference_obj = reference_points_obj

        # query = query.permute(1, 0, 2)
        # query_obj = query_obj.permute(1, 0, 2)
        # query_pos_obj = query_pos_obj.permute(1, 0, 2)
        # bev_embed = bev_embed.permute(1, 0, 2)

        # inter_states_obj, inter_references_obj = self.decoder_obj(
        #     query=query_obj,
        #     key=None,
        #     value=bev_embed,
        #     query_pos=query_pos_obj,
        #     reference_points=reference_points_obj,
        #     reg_branches=reg_branches_obj,
        #     cls_branches=cls_branches_obj,
        #     spatial_shapes=torch.tensor([[bev_h, bev_w]], device=query_obj.device),
        #     level_start_index=torch.tensor([0], device=query_obj.device),
        #     prev_bev = prev_bev,
        #     **kwargs)
        # print(type(object_query_embeds_obj))
        return object_query_embeds_obj
    ...
    def attn_bev_encode(
        ...
        if lidar_feat is not None:
            bs = mlvl_feats[0].size(0)
            bev_queries = bev_queries.view(bev_h, bev_w, bs, -1).permute(2,3,0,1).contiguous()
            ...
            fused_bev = lidar_feat

            # fused_bev = self.fuser([bev_queries, lidar_feat])
            # print(fused_bev.shape)
            # print('#########Transformer Lidar###############')
            # print(stop)
            fused_bev = fused_bev.flatten(2).permute(0,2,1).contiguous()
            bev_queries = fused_bev
            bev_queries = bev_queries.permute(1,0,2)
        # print('#########Transformer Camera###############')
        # print(stop)
...
#For object
        ...
        # print(reference_points_obj.shape)
        # print('######Shape#########')
        # print(reg_branches_obj)
        # print(stop)
        ...
        return bev_embed, inter_states, init_reference_out, inter_references_out, inter_states_obj,init_reference_obj, inter_references_obj
