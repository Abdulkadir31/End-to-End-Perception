...
class MapTR(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
        ...
        loss_inputs_fus = [gt_bboxes_3d_fus, gt_labels_3d_fus, outs1]
        losses_fus = self.pts_bbox_head.loss1(*loss_inputs_fus, img_metas=img_metas)
       
        
        # print(stop)
        return losses, losses_fus
    ...
    def obtain_history_bev(self, imgs_queue,lidar_feat, img_metas_list):
            ...
            for i in range(len_queue):
                ...
                if lidar_feat == None:
                    prev_bev = self.pts_bbox_head(
                        img_feats, None, img_metas=img_metas, prev_bev=prev_bev, only_bev=True)
                else: 
                    prev_bev = self.pts_bbox_head(
                        img_feats, lidar_feat[i], img_metas=img_metas, prev_bev=prev_bev, only_bev=True)
    ...
    def forward_train(self,
                      ...
                      gt_bboxes_3d=None,
                      gt_labels_3d=None,
                      gt_bboxes_3d_fus=None,
                      gt_labels_3d_fus=None,
                      gt_labels=None,
                      gt_bboxes=None,
                      img=None,
                      proposals=None,
        ...
        if self.modality == 'fusion':
            ...
            if len_queue > 1:
                previous_lidar.append(self.extract_lidar_feat(points0))
                previous_lidar.append(self.extract_lidar_feat(points1))
        else:
            previous_lidar = None
        # print('###############lidar#################')
        # print(img_metas)
        ...
        prev_bev = self.obtain_history_bev(prev_img, previous_lidar , prev_img_metas) if len_queue>1 else None
        ...
        losses = dict()
        # losses_obj = dict()
        losses_pts,losses_fus = self.forward_pts_train(img_feats, lidar_feat, gt_bboxes_3d,
                                            gt_labels_3d, gt_bboxes_3d_fus, gt_labels_3d_fus, img_metas,
                                            
                                            gt_bboxes_ignore, prev_bev)

        losses.update(losses_pts)
        losses_fus= dict((key+'_obj', value) for (key, value) in losses_fus.items())
        losses.update(losses_fus)
        # losses_obj.update(losses_fus)
        # print('#######Loss######')
        # print(losses)
        # print('#######Loss######')
        # print(losses_obj)
        # print('##############modified dict######')
        # print(type(losses_fus))
        # losses_fus= dict((key+'_obj', value) for (key, value) in losses_fus.items())
        # print(losses_fus)
        # print('#######Combined Loss#######')
        # losses_temp = dict()
        # losses_temp.update(losses_pts)
        # losses_temp.update(losses_fus)
        # print(losses_temp)
        # print('#######Combined Loss#######')
        return losses
    ...
    def simple_test_pts(self, x, lidar_feat, img_metas, prev_bev=None, rescale=False):
        """Test function"""
        outs, outs1 = self.pts_bbox_head(x, lidar_feat, img_metas, prev_bev=prev_bev)

        bbox_list,bbox_list_obj = self.pts_bbox_head.get_bboxes(
            outs,outs1, img_metas, rescale=rescale)
        ...
        bbox_results_obj = [
            ...
            for bboxes, scores, labels in bbox_list_obj
        ]

        # print('#######for testing purpose#########')
        # print(outs['bev_embed'])
        # print('#######for testing purpose#########')
        # print(outs1['bev_embed'])
        # print('#######for testing purpose#########')
        # print(stop)


        # import pdb;pdb.set_trace()
        return outs['bev_embed'], bbox_results, bbox_results_obj
    ...
    def simple_test(self, img_metas, img=None, points=None, prev_bev=None, rescale=False, **kwargs):
        ...
        # print('########checking#######')
        # print(stop)
        new_prev_bev, bbox_pts, bbox_pts_obj = self.simple_test_pts(
            img_feats, lidar_feat, img_metas, prev_bev, rescale=rescale)
        
        for result_dict, pts_bbox in zip(bbox_list, bbox_pts):
            result_dict['pts_bbox'] = pts_bbox
        
        for result_dict, pts_bbox in zip(bbox_list, bbox_pts_obj):
            result_dict['pts_bbox_obj'] = pts_bbox
